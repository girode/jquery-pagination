$(function () {
        var jqTable = $("div.sf_admin_list > table").pagiNation({
            ...
            'url':                   '<?php echo url_for("pase_interno/getPageAjax") . "?estado={$estado}&pag=" ?>',
            ...
            'pageChangeSuccess': function () {
                ...
                var datosPaginaActual = this.data['personasSeleccionadas'][this.currentPage];
                ...
                var m = this.data['checkMaestro'][this.currentPage];
                if (m) {
        ...
        var $paginatorData = jqTable.pagiNation("getData");
        ...
        $('table').on('change', "input[type='checkbox']:not([id='sf_admin_list_batch_checkbox'])", function(){
            ...
            if(!$paginatorData['personasSeleccionadas'][jqTable.pagiNation("getCurrentPage")]){
                $paginatorData['personasSeleccionadas'][jqTable.pagiNation("getCurrentPage")] = [];
                index = -1;
            } else {
                index = $.inArray(value, $paginatorData['personasSeleccionadas'][jqTable.pagiNation("getCurrentPage")]);
            }   
            ...
            if(this.checked){
                ...
                if (!(index > -1)) {
                    $paginatorData['personasSeleccionadas'][jqTable.pagiNation("getCurrentPage")].push(value);
                }
            // Si destildo, quito elemento
            } else {
                // Si lo encuentra, lo saco
                if (index > -1) {
                    $paginatorData['personasSeleccionadas'][jqTable.pagiNation("getCurrentPage")].splice(index, 1);
                }
            }    
        ...
        */
        ...
        $("#sf_admin_list_batch_checkbox").change(function(){
            ...
            if(chequeado) {
                $paginatorData['checkMaestro'][jqTable.pagiNation("getCurrentPage")] = true;
                if($('#cancel_all_div').is(':hidden'))
                    $('#select_all_div').show();
            }
            else {
                $paginatorData['checkMaestro'][jqTable.pagiNation("getCurrentPage")] = false;
